...
export default function ProductCreationPage() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        toast.error(
          "Failed to load category information, please refresh the browser or contact naveed"
        );
      }
    };
    fetchCategories();
  ...
  const handleSubmit = async () => {
    ...
    // Check for invalid hex codes
    const invalidColors = colors.some((c) => c && !isValidHex(c));
    if (invalidColors) {
      setError("Please enter only valid hex codes for all colors.");
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-md">
        ...
        <Link
          to="/cms/product/table"
          className="inline-block mb-4 bg-indigo-600 text-white px-4 py-2 rounded shadow hover:bg-indigo-700"
        >
          View All Products
        </Link>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-4">
              {productImagePreviews.map((preview, index) => (
                <div
                  key={index}
                  className="relative h-32 rounded overflow-hidden border"
                >
                  <img
                    src={preview}
                    alt={`Preview ${index + 1}`}
                  ...
                  <div className="flex">
                    <input
                      ...
                      className={`flex-1 border px-2 py-1 rounded ${
                        colorErrors[index] ? "border-red-500" : ""
                      }`}
                      placeholder={`Hex Color #${index + 1}`}
                    />
                    <button
                      onClick={() => removeFromArray(index, setColors)}
              ...
              {sizes.map((size, index) => (
                <div key={index} className="flex mt-1">
                  <input
                    value={size}
                    onChange={(e) =>
                      handleArrayChange(index, e.target.value, setSizes)
                    }
                    className="flex-1 border px-2 py-1 rounded"
                    placeholder={`Size ${index + 1}`}
                  />
                  <button
                    onClick={() => removeFromArray(index, setSizes)}
                    className="ml-2 text-red-500"
                  >
                    ✕
                  </button>
